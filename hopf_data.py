...
def hopf_state_gradient_from_metric(
    ...
    *,
    ...
    Boundary-safe version. Each leaf derivative is evaluated as a direct product
    with one local factor differentiated, avoiding tan/cot divisions.
    """
    theta = np.asarray(theta, dtype=float)
    ...
    for leaf in range(dim):
        ...
        for depth, node in enumerate(nodes):
            ...
            value += coeff[j0] * prefix * local_derivative * suffix[depth + 1]
