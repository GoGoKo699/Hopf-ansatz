...
def infer_n_from_theta(theta, case=None):
    """Infer the physical qubit count from a Hopf parameter-vector length.

    The length alone is ambiguous: a complex n-qubit Hopf vector has length
    2^(n+1)-1, which is also the length of a real (n+1)-qubit Hopf vector.
    Pass case="real" or case="complex" whenever the distinction matters.
    """
    L = int(np.asarray(theta).size)
    if L < 1:
        raise ValueError("Hopf theta must contain at least one parameter.")

    m = L + 1
    if m & (m - 1):
        raise ValueError("Hopf theta length must be 2^n - 1 for real or 2^(n+1) - 1 for complex.")

    log_val = int(round(math.log2(m)))

    if case is None:
        raise ValueError("Theta length alone is ambiguous; pass case='real' or case='complex'.")

    if case == 'real':
        n = log_val
        if (1 << n) - 1 != L or n < 1:
            raise ValueError("Real Hopf theta must have length 2^n - 1 with n >= 1.")
        return n

    if case == 'complex':
        n = log_val - 1
        if 2 * (1 << n) - 1 != L or n < 1:
            raise ValueError("Complex Hopf theta must have length 2^(n+1) - 1 with n >= 1.")
        return n
...
def _hopf_node_prefix_factor(theta_mags, i):
    """Signed amplitude entering internal node i before applying theta_i."""
    theta_mags = np.asarray(theta_mags, dtype=float)
    if i < 1:
        raise ValueError("node index i must be one-based and positive")

    pref = 1.0
    for anc, bit in zip(_hopf_ancestors_path(i), _hopf_path_bits(i)):
        th = float(theta_mags[anc - 1])
        pref *= math.sin(th) if bit == 1 else math.cos(th)
    return pref


def partial_derivative_column_real(theta, i):
    """Return d psi(theta) / d theta_i for the real Hopf chart.

    Boundary-safe: this avoids the interior-only tan/cot form, whose removable
    singularities become wrong at theta_i = 0, pi/2, pi, ... .
    """
    theta = np.asarray(theta, dtype=float)
    L = len(theta)
    n = int(round(math.log2(L + 1)))
    if (1 << n) - 1 != L:
        raise ValueError("real Hopf theta must have length 2^n - 1")
    if i < 1 or i > L:
        raise ValueError(f"Index i={i} out of range [1, {L}]")

    pref = _hopf_node_prefix_factor(theta, i)
    if pref == 0.0:
        return np.zeros(1 << n, dtype=float)

    th_tangent = theta_hopf_tangent_state(theta, i, case="real")
    return pref * vector_from_theta(th_tangent, "real")


def jacobian(theta_list, case):
    """Compute the full Hopf-coordinate Jacobian.

    Magnitude-angle columns use the shifted tangent-state construction times the
    signed amplitude entering the differentiated node.  This is algebraically
    identical to the tan/cot formula on the interior and remains correct at
    exact branch-boundary coordinates.
    """
    ...
    if case == 'real':
        ...
        for j0 in range(L):
            ...
            if pref != 0.0:
                th_tangent = theta_hopf_tangent_state(theta, i, case="real")
                J[:, j0] = pref * vector_from_theta(th_tangent, "real")
        return J

    if case == 'complex':
        L_total = len(theta)
        n = int(round(math.log2(L_total + 1))) - 1
        num_mags = (1 << n) - 1
        if L_total != 2 * (1 << n) - 1:
            raise ValueError("complex Hopf theta must have length 2^(n+1) - 1")
        ...
        for j0 in range(num_mags):
            i = j0 + 1
            pref = _hopf_node_prefix_factor(mag_thetas, i)
            if pref != 0.0:
                th_tangent = theta_hopf_tangent_state(theta, i, case="complex")
                J[:, j0] = pref * vector_from_theta(th_tangent, "complex")

        for k in range(1 << n):
            J[k, num_mags + k] = 1j * x[k]
