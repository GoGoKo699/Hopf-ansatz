#!/usr/bin/env python3
"""
diagnose_adam_data.py

Streaming diagnostics for synthetic Adam-baseline CSV files produced by adam_data.py.

Default expected input files in --indir:
    vqe_adam_data_n6.csv,  met_adam_data_n6.csv,
    ...
    vqe_adam_data_n10.csv, met_adam_data_n10.csv
...
because the vector columns can dominate file size. It computes convergence, ranking,
seed-aware completeness, and pathology summaries from scalar columns while optionally checking sampled
vector lengths by counting semicolon-separated entries.
"""
...
def find_input_files(indir: Path, ns: Sequence[int]) -> List[Path]:
    files: List[Path] = []
    for n in ns:
        for base in (f"vqe_adam_data_n{n}.csv", f"met_adam_data_n{n}.csv"):
            p = indir / base
            pgz = indir / (base + ".gz")
            if p.exists():
                files.append(p)
...
def expected_file_list(indir: Path, ns: Sequence[int]) -> List[Path]:
    out = []
    for n in ns:
        out.append(indir / f"vqe_adam_data_n{n}.csv")
        out.append(indir / f"met_adam_data_n{n}.csv")
    return out
...
def build_report(
    ...
    lines: List[str] = []
    lines.append("ADAPTIVE-LINE-SEARCH ADAM BASELINE CSV DIAGNOSTICS")
    lines.append(line())
    ...
    lines.append("- For Möttönen rows, theta/grad are physical post-multiplexing Ry parameters and ideal-PS gradients; for Hopf rows, theta/grad are Hopf coordinates and Hopf-coordinate gradients.")
    lines.append("- These baseline files contain Adam coordinate updates with optional adaptive line search; last_line_evals counts trial cost evaluations, not additional gradient or metric-estimation primitives.")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Diagnose synthetic Adam-baseline CSV files.")
    p.add_argument("--indir", default=".", help="Directory containing vqe_adam_data_n*.csv and met_adam_data_n*.csv files.")
    ...
    p.add_argument("--out", default="adam_data_diagnostics.txt", help="Output diagnostic text file.")
