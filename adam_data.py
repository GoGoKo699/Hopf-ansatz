...
def unit_normalize(x: np.ndarray, *, eps: float = 1e-15) -> np.ndarray:
    """Normalize without changing the global sign.

    Use this for parameter-to-state maps whose Jacobians/gradients are computed
    in the same raw coordinate chart. Applying the sign-fixing normalize() there
    can flip the objective state without flipping the chart derivative.
    """
    nrm = float(np.linalg.norm(x))
    if nrm < eps:
        raise ValueError("Cannot normalize near-zero vector.")
...
def hopf_state_gradient_from_coordinate_gradient(
    ...
    *,
    ...
    Boundary-safe version of J g^{-1} grad_theta. It evaluates each leaf
    derivative by direct products, not by multiplying the current leaf amplitude
    by tan/cot factors.
    """
    theta = np.asarray(theta, dtype=float)
    ...
    for leaf in range(dim):
        ...
        for depth in range(n):
            ...
            th = float(theta[node - 1])
            s = math.sin(th)
            c = math.cos(th)
            nodes.append(node)
            bits.append(bit)
            factors.append(c if bit == 0 else s)
            node = 2 * node + bit

        suffix = np.ones(n + 1, dtype=float)
        for depth in range(n - 1, -1, -1):
            suffix[depth] = factors[depth] * suffix[depth + 1]
        ...
        for depth, node in enumerate(nodes):
            ...
            s = math.sin(th)
            c = math.cos(th)
            local_derivative = -s if bits[depth] == 0 else c
            value += nat[j0] * prefix * local_derivative * suffix[depth + 1]
            prefix *= factors[depth]
        out[leaf] = value
...
def hopf_coordinate_gradient(theta: np.ndarray, euclidean_grad: np.ndarray) -> np.ndarray:
    """Compute J^T euclidean_grad for the real Hopf chart.

    Boundary-safe version. It never rewrites derivatives as tan(theta) or
    cot(theta) times the current amplitude, so exact zero branches still produce
    the correct finite derivative in the newly opened branch.
    """
    theta = np.asarray(theta, dtype=float)
    egrad = np.asarray(euclidean_grad, dtype=float)
    L = theta.size
    n = int(round(math.log2(L + 1)))
    dim = 1 << n
    if (1 << n) - 1 != L:
        raise ValueError("Real Hopf theta length must be 2^n - 1.")
    if egrad.shape != (dim,):
        raise ValueError(f"euclidean_grad must have shape ({dim},), got {egrad.shape}.")

    # subtree_resp[i] = <normalized subtree state rooted at node i | egrad>.
    subtree_resp = np.zeros(L + 1, dtype=float)
    for node in range(L, 0, -1):
        start, mid, _ = hopf._subtree_span(node, n)
        left_child = 2 * node
        right_child = left_child + 1
        left_resp = subtree_resp[left_child] if left_child <= L else egrad[start]
        right_resp = subtree_resp[right_child] if right_child <= L else egrad[mid]
        th = float(theta[node - 1])
        subtree_resp[node] = math.cos(th) * left_resp + math.sin(th) * right_resp

    # prefix[node] is the signed incoming amplitude at that node.
    grad = np.zeros(L, dtype=float)
    prefix = np.zeros(L + 1, dtype=float)
    prefix[1] = 1.0
    for node in range(1, L + 1):
        ...
        s = math.sin(th)
        c = math.cos(th)
        grad[node - 1] = prefix[node] * (-s * left_resp + c * right_resp)
        if left_child <= L:
            prefix[left_child] = prefix[node] * c
        if right_child <= L:
            prefix[right_child] = prefix[node] * s
...
def mottonen_state_from_phi(phi: np.ndarray, n: int) -> np.ndarray:
    ...
    return unit_normalize(hopf.vector_from_theta(alpha, "real"))
...
def mottonen_adjoint_ps_equivalent_gradient(task: BaseTask, phi: np.ndarray, n: int) -> np.ndarray:
    ...
    """
    alpha = mottonen_phi_to_hopf_theta(phi, n)
    psi = unit_normalize(hopf.vector_from_theta(alpha, "real"))
...
def run_hopf_adam(task: BaseTask, steps: int, args: argparse.Namespace, *, run_seed: int, seed_index: int) -> List[Dict[str, object]]:
    ...
    def state_from(th: np.ndarray) -> np.ndarray:
        return unit_normalize(hopf.vector_from_theta(project(th), "real"))
