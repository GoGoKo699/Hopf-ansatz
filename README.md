...
Official code and reproducibility package for:

**A Compass on the Quantum State Sphere:  
...
## What is the Hopf ansatz?

The Hopf ansatz is a binary-tree variational quantum circuit for arbitrary normalized real and complex pure states. It provides:

- a forward state-preparation map;
- an explicit inverse map from amplitudes to circuit angles;
- a diagonal pullback metric;
- preparable normalized coordinate tangents;
- structured quantum gradient access; and
- geometry-aware pure-state optimization.

This repository contains the reference implementation and reproducibility material supporting the paper, including coordinate maps, inverse maps, Jacobians, metrics, tangent-state constructions, optimization tests, finite-shot checks, resource-count safeguards, and Qibo circuit demonstrations.
...
| `finite_shot_sanity_check.py` | Fixed-state finite-shot check of the symmetric signed-branch Hopf gradient estimator. It compares sampled full coordinate gradients with an exact tree-gradient reference, prints statistical diagnostics, and generates its CSV and requested plot formats. |
| `finite_shot_sanity.png` | Included finite-shot estimator summary for the default `n=6`, 50-trial experiment. |
...
The CSV is a generated artifact and is not intended to be tracked in source control. The PNG contains one two-panel final-gap figure with the same meaning, optimizer order, plotting conventions, and color code as the upper row of the main real-Hopf summary figure.
...
To generate both PNG and PDF summaries, pass:
